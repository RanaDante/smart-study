...
if(isset($_POST['submit'])){
    ...
    if($email_chk_v == false){
       ...
       if( mysqli_num_rows($result_chk) > 0 ){
        $errors['same-email'] = "<span style='color:red;font-size:14px;text-align:center;'> Email already exist</span>";
       }
        if($email_chk_v == false){
            $errors['email']= "Make sure its a valid email syntax";
        }else{
                $error_em= " ";
        }
    ...
    if($c_password_chk_v == false){
        $errors['c-pass']= "password field can not be empty";
    }elseif($c_password_chk_v == true){
        if($c_password != $password){
            $errors['c-pass']= "Password must match";
        }
    }
